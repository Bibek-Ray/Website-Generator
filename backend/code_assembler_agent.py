...
class CodeAssemblerAgent:
    ...
    def assemble_code(self, section_layout, style_info, copy_info, website_type):
        if self.llm:
            prompt = f"""
            ...
            # Fallback to deterministic, template-based approach.
            html = [f"""<!DOCTYPE html>
            <html>
            <head>
                <meta charset="UTF-8" />
            ...
            if website_type == "blog":
                html.append("<header><h1>Welcome to the Blog</h1></header>")
            elif website_type == "ecommerce":
                html.append("<header><h1>Our Products</h1></header>")
